...
public class MessageController {
    ...
    public void getMessage(@DestinationVariable String roomId, Message message) {
        if (roomId.equals(SECRET_ROOM)) {
            logger.info("Message {} in room {} will not be saved", message.messageStr(), roomId);
            return;
        }
        logger.info("get message:{}, roomId:{}", message, roomId);
        saveMessage(roomId, message).subscribe(msgId -> logger.info("message send id:{}", msgId));

        Message messageToSend = new Message(HtmlUtils.htmlEscape(message.messageStr()));
        template.convertAndSend(String.format("%s%s", TOPIC_TEMPLATE, roomId), messageToSend);
        template.convertAndSend(String.format("%s%s", TOPIC_TEMPLATE, SECRET_ROOM), messageToSend);
    }
